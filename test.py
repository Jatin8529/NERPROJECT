...
def main():
    ...
    # Add argument for CPU only
    parser.add_argument("--n_gpu", type=int, default=0, help="Number of GPUs to use. Set to 0 for CPU only.")

    args = parser.parse_args()
    sanity_checks(args)

    # ARGS #
    init_gpu_params(args)  # Ensure this function handles CPU-only settings
    set_seed(args)
    ...
    if args.is_master:
        if os.path.exists(args.dump_path):
            if not args.force:
                raise ValueError(
                    f"Serialization dir {args.dump_path} already exists, but you have not specified whether to overwrite"
                    " it. Use `--force` if you want to overwrite it."
                )
            else:
    ...
    # Ensure that `student` is moved to CPU
    if args.n_gpu > 0:
    ...
    # DISTILLER #
    torch.cuda.empty_cache()  # This is a no-op on CPU but harmless
    distiller = Distiller(
